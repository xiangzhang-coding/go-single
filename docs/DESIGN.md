...
[4] 预扣成功 → 发 MQ → 返回"排队中"
[5] 消费者：建 DB 秒杀订单（(user_id, activity_id) 唯一约束兜底重复投递/并发，重复即幂等成功）→ 同事务扣活动库存
...
- **幂等两段式**：Redis 键挡预扣请求的重复提交（先于 Lua 抢占，见时序 [2]）；DB 唯一约束挡落单重复（消费者直接建单，`(user_id, activity_id)` 唯一约束命中视为幂等成功，不重复扣库存）——预扣成功绝不丢单
