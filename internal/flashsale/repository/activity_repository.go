...
type ActivityRepository interface {
	...
	UpdateInTx(ctx context.Context, tx *transaction.Handle, a *model.Activity) error
	GetByID(ctx context.Context, id int64) (*model.Activity, error)
	GetByIDForUpdate(ctx context.Context, tx *transaction.Handle, id int64) (*model.Activity, error)
	...
	DeductStock(ctx context.Context, tx *transaction.Handle, id int64, quantity int) (bool, error)
	// RestoreStock 事务内回补活动库存（stock + quantity）。供秒杀订单取消
	// 在订单事务内回补（与状态迁移同事务，MySQL 为落单事实源）。
	RestoreStock(ctx context.Context, tx *transaction.Handle, id int64, quantity int) error
}
