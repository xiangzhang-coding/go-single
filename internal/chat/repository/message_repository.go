...
type ConversationRepository interface {
	...
	Ensure(ctx context.Context, tx *gorm.DB, c *model.Conversation) error
	...
	TouchLastMessage(ctx context.Context, tx *gorm.DB, key string, messageID int64) error
}

// MessageRepository 消息数据访问接口。
type MessageRepository interface {
	// Create 落库消息；同 (sender_id, client_request_id) 已有行返回 ErrMessageDuplicate。
	Create(ctx context.Context, tx *gorm.DB, m *model.Message) error
