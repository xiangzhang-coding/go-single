...
| limiter | 令牌桶 + Redis 固定窗口计数 | 全局令牌桶中间件（`golang.org/x/time/rate`，单实例）；`RedisCounter`（INCR+TTL Lua，跨请求状态，fail-closed） |
...
| cache | Redis 缓存层（ADR-0003 seam） | 接口隔离 go-redis；`Eval` 原子执行 Lua 脚本（学习点，业务模块持有脚本内容） |
...
| file | MinIO 私有桶上传代理 | `POST /api/files`（multipart 字段 "file"，Bearer）；**魔数嗅探**类型白名单（png/jpeg/webp/gif）+ ≤5MB；桶私有（匿名探测防暴露存在性）；前端不直连 MinIO |
...
| 指标 | 类型 | 维度 |
| --- | --- | --- |
| seckill_prededuct_total | CounterVec | result（success/fail）——秒杀 Lua 预扣 |
