...
| limiter | 令牌桶 + Redis 固定窗口计数 | 登录/注册按可信来源 IP 与 MySQL 排序权重账号键分别计数；秒杀全局令牌桶（`golang.org/x/time/rate`，单实例）；`RedisCounter` 经 `IncrementFixedWindow` 类型化缓存能力完成 INCR+TTL（跨请求状态，fail-closed） |
